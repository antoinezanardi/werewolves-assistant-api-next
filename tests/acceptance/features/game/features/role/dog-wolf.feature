...
Feature: 🐶 Dog Wolf role
  ...
  Scenario: 🐶 Dog Wolf side is randomly chosen if no choice is made with good option

    When a created game with options described in file no-sheriff-option.json, dog-wolf-side-randomly-chosen-option.json and with the following players
      | name    | role     |
      | Antoine | dog-wolf |
      | Benoit  | villager |
      | Cecile  | villager |
  ...
  Scenario: 🐶 Dog Wolf can't choose a side if it must be randomly chosen
